...
<b>Warning</b>: this is not compatible with OSX Lion - don't even try. We're having problems with Snow Leopard and Leopard, lately, too. There may be a problem when trying to run this on early Intel Macs. So people are swearing at Graphviz on Twitter. It is difficult for us to keep up, because of the complexity of both Graphviz (which depends on a lot of GNU libraries, all the way down to glib2) and OSX itself. (We suspect a recent minor patch to OSX changed the way one of the OSX frameworks talks to Graphviz for font scaling.) At this point you may be better off using Homebrew or Macports to install Graphviz by compiling the source (this requires XCode, so allow an afternoon for the process. Anyone that wants to help improve this situation is more than welcome. 
<p>
