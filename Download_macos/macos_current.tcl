...
foreach {package platforms} $packages_platforms {
    ...
    puts $fout "<B>Note</B>: There is no Snow Leopard port yet. There's no need"
    puts $fout "to file a bug report about this. We already know that the"
    puts $fout "Leopard port does not install cleanly on Snow Leopard."
    puts $fout "We expect to provide a Snow Leopard port by mid October."
    puts $fout "If you are impatient, try installing graphviz-gui"
    puts $fout "from Macports. Also, if a volunteer can try building"
    puts $fout "the Snow Leopard port from the current source, enabling the"
    puts $fout "appropriate Mac-specific configure options and the optional"
    puts $fout "cairo, gd, and quartz drivers, and report the results to us,"
    puts $fout "that might be a real help. Please report the results to"
    puts $fout "gviz-bugs \[ a t \] research.att.com"
    puts $fout "<tbody>"
    puts $fout "<tr>"
